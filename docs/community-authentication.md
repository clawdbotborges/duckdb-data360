...
The repository-root [`.github/workflows/MainDistributionPipeline.yml`](../.github/workflows/MainDistributionPipeline.yml) invokes DuckDB's official extension distribution and code-quality workflows for DuckDB v1.5.5. The centralized Community Extensions pipeline performs the final multi-platform build and signs accepted release artifacts; no signing key is stored in this repository. Local CTest and SQLLogic commands remain documented in the project README and are also exercised by the extension build/test pipeline.
